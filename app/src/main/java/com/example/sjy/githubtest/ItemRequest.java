 ...
 **/
public class ItemRequest extends StringRequest {   //stringrequest : 문자열을 결과로 받는 요청 정보, 서버로부터 문자열 데이터를 얻을 목적으로 사용하는 클래스

    //서버 url 설정(php파일 연동)
    final static  private String URL="http://polarbear1022.dothome.co.kr/iteminfo.php";
    private Map<String,String> map;

    public ItemRequest(String uid, Response.Listener<String> listener){
        /**첫번째 매개변수 : HTTP Method 로서 GET이나 POST로 지정**/
        /**두번째 매개변수 : 서버 URL 정보**/
        /**세번째 매개변수 : 결과 콜백, 서버로부터 수신된 문자열 데이터를 전달할 목적으로 세 번째 매개변수로 지정된 콜백의 OnResponse() 함수가 자동으로 호출됨(MainActivity.java에 있음), 이 함수의 매개변수로 서버 수신 문자열이 전달됨**/
        /**네번째 매개변수 : 에러 콜백**/
